...
def get_similar_products():
    ...
    if category:
        ...
        products = Product.find_products_of_same_category(name)
    elif price:
        products = Product.find_highest_price_product_by_category(category)
    else:
...
def get_highest_price(category):
    ...
    app.logger.info("Returning Information regarding highest price products in category: %s", category)
    return make_response(jsonify([product.serialize() for product in products]), status.HTTP_200_OK)
